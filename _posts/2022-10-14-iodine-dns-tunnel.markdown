...
I want to let you know upfront, before you try this, that it will be extremely slow. You're not going to see gigabit network speeds, let alone megabit speeds. You'll probably only see a few kilobytes per second, at best. Either way, this could be useful in some situations.

## How?

[Iodine][iodine] creates a network tunnel by acting as a domain name server.

When your local device sends a dns request for a domain name, it will travel to your carriers dns server, which will then look up the name server associated with that domain. Your carriers dns server will then query that name server and return the result. This is known as a recursive dns server. We will configure our domain's name server to be our Iodine server.

By using a custom domain name server;
...
> `t1` stands for `Tunnel 1`, and `t1ns` stands for `Tunnel 1 Name Server`. You can use anything, as long as it has the format `{any}.mydnstunnel.com` and `{any}ns.mydnstunnel.com`.

Before proceeding any further, you should check that your 4G/LTE connection is able to fetch the dns records you just configured. Try with your carrier dns server, probably used by default, otherwise try through Google's dns servers to see if your carrier allows access to arbitrary dns servers.
...
Next up is to connect your local device to your carrier network through your 4G/LTE modem (or whatever device you chose to use).

Then download and build Iodine for your local device. I am using macOS, and didn't need to install any dependencies, but your setup might be slightly different.
...
Now that you have Iodine on your local device, to establish a dns tunnel to the remote server.
...
> Note: Your local device IP might be different. Check the output after running Iodine for the correct IP address.
...
For example, in my network I have a MikroTik RouterBoard as my main DNS server. You should configure this to have a low TTL for dns caching, or change your local device dns servers to your carrier dns servers to remove the extra dns server.
...
- You can ssh directly into the remote server via the dns tunnel ip.
- Maybe investigate running a [wireguard][wireguard] server instead of using a socks5 proxy.
